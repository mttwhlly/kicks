      ...
      <>
        <Box className='bg-neutral-100 pt-[60px]'>
          <Box className='max-w-3xl mx-auto gap-4 mt-12 py-8 flex flex-col'>
            ...
            <h1 className="text-4xl font-bold">{`${firstName ? firstName : ''} ${middleName ? middleName : ''} ${lastName ? lastName : ''} ${title ? title : ''}`}</h1>
            {/* <p>{data.pronouns}</p> */}
        ...
        <h2 className="text-xl font-bold mt-4">{`Health Plan Network (${poData.length})`}</h2>
          <Box className="flex flex-row gap-4 overflow-x-scroll">
            <Stack direction="row" spacing={2}>
              {poData.map((po) => (
                <Card className="px-2 py-8 bg-neutral-100 rounded text-center hover:bg-neutral-200 transition-colors my-2 w-3xs font-semibold shadow-none"><Link to={`/organization/${po.id}/map`}>{po.name}</Link></Card>
              ))}
            </Stack>
          </Box>
