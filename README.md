...
This template follows a clean architecture approach, separating concerns into different layers:

- **Domain**: Contains the core business logic and entities.
- **Data**: Handles data fetching and storage.
- **Presentation**: Contains the UI components and presentation logic.
- **Application**: Manages the application state and coordinates between the domain and presentation layers.
- **Infrastructure**: Contains the implementation details, such as API clients and third-party libraries.
- **Shared**: Contains shared components and utilities that can be used across different layers.
...
- `app` - **Frameworks & Drivers Layer** - pages,
...
- **Frameworks & Drivers**: keeps all the UI framework functionality, and
  everything else that interacts with the system (ex. Azure Functions, Entra ID
  webhooks etc...). Route Handlers, Components (Server and Client), Pages, Design System, etc...
...
- **Entities**: where the _Models_ and _Errors_ are defined.
  ...
  - **Errors**:
    ...
    - We `catch` errors that are coming from other libraries, and convert those 
      errors to our own errors.
