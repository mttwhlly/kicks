...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const practitionerApi = `${apiUrl}/api/nova/practitioner?practitionerID=${params.id}`;
  const poApi = `${apiUrl}/api/nova/po?practitionerID=${params.id}`;
  const locationApi = `${apiUrl}/api/nova/location?practitionerID=${params.id}`;
  const specialtyApi = `${apiUrl}/api/nova/specialty?practitionerID=${params.id}`;
  const languageApi = `${apiUrl}/api/nova/language?practitionerID=${params.id}`;
...
export default function Practitioner({ loaderData }: Route.ComponentProps) {
  ...
  const primaryPracticeLocation = locationData.find(
    (location) => location.officeType === 'Primary Practice'
  );
    ...
    <>
      <Box className="bg-neutral-100 pt-[60px]">
        <Box className="max-w-3xl mx-auto gap-4 mt-12 py-8 flex flex-col">
          ...
          <h1 className="text-4xl font-bold">{`${firstName ? firstName : ''} ${
            middleName ? middleName : ''
          } ${lastName ? lastName : ''} ${title ? title : ''}`}</h1>
          {/* <p>{data.pronouns}</p> */}
      ...
      <Box className="max-w-3xl mx-auto gap-4 flex flex-col my-4">
        ...
        <Box className="flex flex-row gap-4 overflow-x-scroll">
          <Stack direction="row" spacing={2}>
            {poData.map((po) => (
              <Card className="px-2 py-8 bg-neutral-100 rounded text-center hover:bg-neutral-200 transition-colors my-2 w-3xs font-semibold shadow-none">
                <Link to={`/organization/${po.id}/map`}>{po.name}</Link>
              </Card>
            ))}
          </Stack>
        </Box>
        ...
        <p>
          <PublicIcon />{' '}
          <a href={`https://${data.contactInfo.website}`}>
        ...
        <Card className="p-4 my-2">
          ...
          <p>
            {primaryPracticeLocation.city}, {primaryPracticeLocation.state}{' '}
            {primaryPracticeLocation.zip}
          </p>
        ...
        {locationData
          .filter(
            (location) =>
              location.officeType.toLowerCase() !== 'primary practice'
          )
          .map((location) => (
            <Card className="p-4 my-2">
        ...
        <Accordion>
          <AccordionSummary expandIcon={<ArrowDropDownIcon />}>
            <h2 className="text-lg font-bold">
              Specialties (
              {specialtyData.filter((item) => item.name !== '').length})
            </h2>
          </AccordionSummary>
          <AccordionDetails>
        ...
        <Accordion>
          ...
          <AccordionDetails>
            {languageData.map((language) => {
              return (
                <p key={language.id}>
                  {language.name} ({language.skillName})
                </p>
              );
            })}
          </AccordionDetails>
        ...
        <Card className="p-4 my-2">
          {practitionerData.nationalProviderId ??
            'No National Provider Id (NPI) found'}
        </Card>
      </Box>
    </>
  );
